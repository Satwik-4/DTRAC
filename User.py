...
# python3 User.py --unique-name user1 --address 0xd58BE3F0DDa5C554Af8bFF8417d39C3b417FbeA3 --rpc-endpoint "http://127.0.0.1:7545"
# python3 User.py --unique-name user2 --address 0x45068dEcbeE22C3B6cCf373b4C3Fc1114D01B067 --rpc-endpoint "http://127.0.0.1:7545"
...
assert user_addr is not None, "You must set PRIVATE_KEY environment variable"
assert user_addr.startswith("0x"), "Private key must start with 0x hex prefix"

account: LocalAccount = Account.from_key(user_addr)
...
for validator in validatorsList:
	i = 1
	validator_dict.setdefault(validator[i], i)
	i = i+1
...
def CredentialRequest(title, vcerts, combination, public_m = []): #should be encoded public_m
	...
	tx_hash = request_contract.functions.RequestCred(title, send_vcerts, send_cm, send_compressed_cipher, send_hp, send_hr, send_bo, pi_s, pi_o, send_compressed_G2Points, str_public_m).transact({'from':account.address})
	return Lambda, os
...
def RequestService(credential, user_addr):
	...
	# original_attr = []
	# schemaOrder = downloadSchemaOrder(credential["title"])
	# for key in schemaOrder:
	# 	original_attr.append(credential["attributes"][key])
	...
	tx_hash = verify_contract.functions.VerifyCred(title, send_theta, str_public_m, send_Aw, send_aggr, disclose_index, str_disclose_attr, disclose_attr_enc, _timestamp).transact({'from':account.address})
