...
# python3 SP.py --title 'Loan Service' --name Bank --address 0xa94b38500643F1B0ceD57AEbf732A5Bf8385402B --verify-address 0x73AeB07bB4230015f18178e67717702306DCf2a0 --rpc-endpoint "http://127.0.0.1:7545" --accepts 'Loan Credential'
...
assert SP_addr is not None, "You must set PRIVATE_KEY environment variable"
assert SP_addr.startswith("0x"), "Private key must start with 0x hex prefix"

account: LocalAccount = Account.from_key(SP_addr)
...
for cur_title in acceptable_ACs:
	...
	tx_hash = verify_contract.functions.setPolicy(cur_title, policy).transact({'from':account.address})
