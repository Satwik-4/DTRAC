...
# python3 Validator.py --title "Loan Credential" --id 1 --address 0x8C80AEd02863dead1ddFd267fE572162ae69Cea6 --rpc-endpoint "http://127.0.0.1:7545"
# python3 Validator.py --title "Loan Credential" --id 2 --address 0x04E93De284Db2ABB786562308BA247D0CA7CBCde --rpc-endpoint "http://127.0.0.1:7545"
# python3 Validator.py --title "Loan Credential" --id 3 --address 0x24Bd2729d68ECeAC9C6743b51ca5ce0bFc99d09d --rpc-endpoint "http://127.0.0.1:7545"
...
assert validator_address is not None, "You must set PRIVATE_KEY environment variable"
assert validator_address.startswith("0x"), "Private key must start with 0x hex prefix"

account: LocalAccount = Account.from_key(validator_address)
...
f.close()
validatorsList.append((args.id, account.address))
...
def issuePartialCredentials(pending_requests_count, served_count, sk):
	...
	tx_hash = issue_contract.functions.SendBlindSign(args.title, sender, send_h, send_t).transact({'from':account.address})
