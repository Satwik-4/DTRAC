...
# python3 Opener.py --title "Loan Credential" --id 1 --ip 127.0.0.1 --port 8001 --address 0x245433282Ecd57f701D4c07C895F4d40Ad570A3D --rpc-endpoint "http://127.0.0.1:7545"
# python3 Opener.py --title "Loan Credential" --id 2 --ip 127.0.0.1 --port 8002 --address 0xB09f5265BC0C89f80F114cA299763c58f83caE78 --rpc-endpoint "http://127.0.0.1:7545"
# python3 Opener.py --title "Loan Credential" --id 3 --ip 127.0.0.1 --port 8003 --address 0xdE56Bc4C73DD47fc9FFdee82C2B59A856D14FB58 --rpc-endpoint "http://127.0.0.1:7545"
...
opener_address = args.address

w3 = Web3(Web3.HTTPProvider(args.rpc_endpoint, request_kwargs = {'timeout' : 300}))

assert opener_address is not None, "You must set PRIVATE_KEY environment variable"
assert opener_address.startswith("0x"), "Private key must start with 0x hex prefix"

account: LocalAccount = Account.from_key(opener_address)
...
f.close()
openersList.append((args.id, account.address))
...
def openingThread():
	...
	while True:
		...
		tx_hash = opening_contract.functions.SendOpeningInfo(args.title, opening_session_id, send_open_shares).transact({'from': account.address})
