...
# python3 ProtocolInitiator_AnonymousCredentials.py --title "Loan Credential" --address 0x5CB76baeE21095597486690A0AD3322C7A2d0Cb2 --validator-addresses 0x8C80AEd02863dead1ddFd267fE572162ae69Cea6 0x04E93De284Db2ABB786562308BA247D0CA7CBCde 0x24Bd2729d68ECeAC9C6743b51ca5ce0bFc99d09d --opener-addresses 0x245433282Ecd57f701D4c07C895F4d40Ad570A3D 0xB09f5265BC0C89f80F114cA299763c58f83caE78 0xdE56Bc4C73DD47fc9FFdee82C2B59A856D14FB58 --rpc-endpoint "http://127.0.0.1:7545"
...
assert organization_address is not None, "You must set PRIVATE_KEY environment variable"
assert organization_address.startswith("0x"), "Private key must start with 0x hex prefix"

account: LocalAccount = Account.from_key(organization_address)
...
tx_hash = params_contract.functions.set_params(args.title, encoded_hs, encoded_alpha, encoded_g1_beta, encoded_beta, encoded_opk, combinations, dependency, encoded_include_indexes, public_m_encoding).transact({'from': account.address})
...
for opener_addr in opener_addresses:
	tx_hash = opening_contract.functions.addOpener(opener_addr).transact({'from':account.address})
	w3.eth.waitForTransactionReceipt(tx_hash)

for validator_addr in validator_addresses:
	tx_hash = issue_contract.functions.addIssuer(validator_addr).transact({'from':account.address})
	w3.eth.waitForTransactionReceipt(tx_hash)